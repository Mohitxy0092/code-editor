  ...
  useEffect(() => {
    async function setupContainer() {
      ...
      try {
        ...
        // Step-3 Install dependencies with timeout
        if (terminalRef.current?.writeToTerminal) {
          terminalRef.current.writeToTerminal(
        ...
        const npmTimeout = new Promise((_, reject) =>
        ...
        try {
          ...
          if (
            ...
            error.message === "npm install timeout"
          ) {
            if (terminalRef.current?.writeToTerminal) {
              terminalRef.current.writeToTerminal(
                "\n⚠️ npm install is taking too long. Skipping...\n",
              );
            }
            // Continue without waiting for npm install
          } else {
            throw error;
          }
        }
